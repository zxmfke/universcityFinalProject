...
struct ip4head
{
	u_int8_t  ip4_header_length:4,ip_version:4;
	u_int8_t  ip4_tos;
	u_int16_t ip4_length;
	u_int16_t ip4_id;
	u_int8_t  ip4_flag;
	u_int16_t ip4_off;
	u_int8_t  ip4_ttl;
	u_int8_t  ip4_protocol;
	u_int16_t ip4_checksum;
	struct in_addr ip4_souce_address;
	struct in_addr ip4_destination_address;
};

struct ip6head 
{
	u_int8_t  ip_version:6;
	u_int8_t  ip6_traffic;
	u_int16_t ip6_label;
	u_int16_t ip6_length;
	u_int8_t  ip6_next_header;
	u_int8_t  ip6_limits;
	struct in6_addr ip6_souce_address;
	struct in6_addr ip6_destination_address;
};
...
struct tcphead
  {
    ...
    u_int8_t  th_off:4;         /* data offset 資料開始的位置*/
    u_int8_t  th_flags;
	u_int8_t  th_rs;            /* reserved */
    u_int16_t th_win;           /* window 接收端能夠接收的位元組的大小*/
